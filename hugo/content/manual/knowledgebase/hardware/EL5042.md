...
```
4. After this voltage setting is changed, the slave needs to go to INIT state and back to OP. This is only needed the first time 9V is set. This needs to be done with the EtherCAT tool:
...
### Mounting of scale
Issues with LA11 encoder could be related to sacale mounted upside down (the two tracks, incremental and absolute must teh correct sensor in teh reading head ). In this case LED on reading is flashing green, or flashing red, or solid red (all three cases have been observed).
The diagnostcis from EL5042 looks like below, which indicates its a communication problem but it's actually a mechanical:
```
...
```
When the magnetic strip is correctly mounted the led of the reading head is in the lower part of the text on the strip (RLS).
The the reading head also needs to be aligned, see manual.

### Verification of scale

RLS supplies a credidcard sized inspection tool for the magnetic strips. By this tool you can verify that the strip has two tracks with proper magnetization.
In a few, very seldom, usecases it has been found that the strip has be totally de-magnetized. In that case a the  magnetic strip needs to be replaced.
